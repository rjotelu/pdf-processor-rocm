...
static int init_lib() {
    ...
    for (int i = 0; paths[i]; i++) {
        gpu_lib = dlopen(paths[i], RTLD_NOW | RTLD_LOCAL);
        if (gpu_lib) break;
    }
...
int check_cuda() {
    ...
    return cuda_dev_count_fn() > 0;
}

int do_batch_crop(const char* path, int x1, int y1, int x2, int y2, int wid) {
    if (!init_lib() || !batch_crop_fn) return 0;
    return batch_crop_fn(&path, 1, x1, y1, x2, y2, wid);
}
*/
import "C"
...
func main() {
	...
	flag.IntVar(&cfg.PDFWorkers, "pdf-workers", 2, "Concurrent PDF conversions")
	flag.IntVar(&cfg.Workers, "workers", runtime.NumCPU(), "Parallel workers")
	flag.IntVar(&cfg.DPI, "dpi", 300, "Render DPI")
	...
	fmt.Println("================================================================================")
	fmt.Println("         MASSIVE PARALLEL PDF TO CROPPED MARKDOWN CONVERTER (CUDA)")
	fmt.Println("================================================================================")
	fmt.Printf("[*] Root: %s | Workers: %d | GPU: NVIDIA CUDA\n", cfg.RootDir, cfg.Workers)
	fmt.Println("--------------------------------------------------------------------------------")
...
func checkDeps() {
	...
	cudaAvailable := C.check_cuda() == 1
	if cudaAvailable {
		fmt.Println("[✓] CUDA GPU detected and loaded")
	} else {
		fmt.Println("[!] CUDA GPU library not found (using CPU fallback for cropping)")
	}
}
...
func cropImages(cfg Config, pngFiles []string) int {
	...
	for w := 0; w < cfg.Workers; w++ {
		...
		go func() {
			defer wg.Done()
			for j := range jobs {
				if err := cropImage(j.path, cropRect); err == nil {
...
func cropImage(path string, rect image.Rectangle) error {
