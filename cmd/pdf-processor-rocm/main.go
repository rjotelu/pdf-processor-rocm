...
/*
#cgo rocm CFLAGS: -I../../gpu_lib
#cgo rocm LDFLAGS: -L../../gpu_lib -lgpu_crop_rocm -lpthread

#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>

static int rocm_dev_count() { return 1; }
*/
...
func main() {
	...
	fmt.Println("================================================================================")
	fmt.Println("         MASSIVE PARALLEL PDF TO CROPPED MARKDOWN CONVERTER (ROCM)")
	fmt.Println("================================================================================")
	fmt.Printf("[*] Root: %s | Workers: %d | GPU: AMD ROCm\n", cfg.RootDir, cfg.Workers)
	fmt.Println("--------------------------------------------------------------------------------")
...
func checkDeps() {
	...
	fmt.Println("[✓] ROCm GPU detected (stub library loaded)")
}
...
func discoverPDFs(root string) ([]*PDFTask, error) {
	...
	err := filepath.WalkDir(root, func(path string, d fs.DirEntry, err error) error {
		...
		tasks = append(tasks, &PDFTask{
			ID: len(tasks) + 1,
			PDFPath: path, BaseName: base, ParentDir: parentDir,
...
func processPDF(cfg Config, idx, total int, task *PDFTask, rendered, cropped, ocr *int64) {
	...
	fmt.Print("  Crop(GPU)...")
	croppedCount := cropImagesROCm(cfg, pngFiles)
...
func cropImagesROCm(cfg Config, pngFiles []string) int {
	...
	for w := 0; w < cfg.Workers; w++ {
		...
		go func() {
			defer wg.Done()
			for j := range jobs {
				if err := cropImageROCm(j.path, cropRect); err == nil {
...
func cropImageROCm(path string, rect image.Rectangle) error {
...
func ocrImages(task *PDFTask, pngFiles []string, cfg Config) []*PageResult {
	...
	type job struct{ pageNum int; filePath string }
	jobs := make(chan job, len(pngFiles))
